...
const HairGoalsContainer = () => {
    ...
    const handleDelete = function (id) {
        ...
        const url = '/users/' + id;
        request.delete(url).then(() => (window.location = '/user-details'));
    };
    ...
    const handleUpdate = function (user) {
        const request = new Request();
        request.patch('/users/' + user.id, user).then(() => {
            ...
            <>
                ...
                <Switch>
                    ...
                    <Route
                        path="/user-details/1"
                        render={() => (
                            <UserData theOneUser={users[0]} loaded={loaded} />
                        )}
                    />
                    <Route
                        ...
                        render={() => (
                            <AddMeasurement
                                theOneUser={users[0]}
                                loaded={loaded}
                                onNewAddedMeasurement={handleAddMeasurement}
                            />
                        )}
                    />
